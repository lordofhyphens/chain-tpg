...
void BDDCircuit::to_bdd() 
{
  ...
  {
    const auto& node = netlist.at(i);
    BDD result;
    if(node.type == LogicType::Input)
    {
      result = manager.bddVar(i);
      manager.bddSetPiVar(i);
      bdd_netlist.emplace_back(std::move(result));
      continue;
    }
    if(node.type == LogicType::DFF)
    {
      result = manager.bddVar(i);
      manager.bddSetNsVar(i);
      bdd_netlist.emplace_back(std::move(result));
      continue;
    }
    // catch everything else
    assert(node.nfi() > 0);
    auto pos = std::distance(netlist.cbegin(), std::find(netlist.cbegin(), netlist.cend(), node.fin.at(0)));
    result = bdd_netlist.at(pos);
    switch(node.type)
    {
      case LogicType::Buff:
      case LogicType::DFF_in:
        break;
      case LogicType::Not:
        {
          result = !result;
    ...
    for (size_t j = 1; j < node.nfi(); j++)
    {
      pos = std::distance(netlist.cbegin(), std::find(netlist.cbegin(), netlist.cend(), node.fin.at(j)));
      switch(node.type)
      {
        case LogicType::And:
          result = result.And(bdd_netlist.at(pos)); break;
        case LogicType::Nand:
          result = result.Nand(bdd_netlist.at(pos)); break;
        case LogicType::Or:
          result = result.Or(bdd_netlist.at(pos)); break;
        case LogicType::Nor:
          result = result.Nor(bdd_netlist.at(pos)); break;
        case LogicType::Xor:
          result = result.Xor(bdd_netlist.at(pos)); break;
        case LogicType::Xnor:
          result = result.Xnor(bdd_netlist.at(pos)); break;
        default:
          break;
      }
    }
